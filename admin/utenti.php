...
/**
 * ADMIN/UTENTI.PHP
 * Il blocco POST sta PRIMA di qualsiasi include per evitare
...
// ── Verifica admin PRIMA dell'HTML ────────────────────────────────────
...
$chkAdmin->execute();
$meRow = $chkAdmin->get_result()->fetch_assoc();
$chkAdmin->close();
...
// ── Helper admin_log (ridefinita qui se non già disponibile) ──────────
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($target_id === (int)$_SESSION['id_utente']) {
        header('Location: /admin/utenti.php?msg=selfAction'); exit;
    }
    $chk = $conn->prepare('SELECT role FROM utente WHERE id = ? LIMIT 1');
    $chk->bind_param('i', $target_id); $chk->execute();
    $tRow = $chk->get_result()->fetch_assoc(); $chk->close();
    if ($tRow && $tRow['role'] === 'admin') {
        header('Location: /admin/utenti.php?msg=adminProtected'); exit;
    }
    switch ($action) {
        case 'banna':
            $motivo = trim($_POST['motivo'] ?? '') ?: 'Violazione termini';
            $s = $conn->prepare('UPDATE utente SET bannato=1, ban_motivo=? WHERE id=?');
            $s->bind_param('si', $motivo, $target_id); $s->execute(); $s->close();
            admin_log($conn, 'ban_utente', 'utente', $target_id, ['motivo' => $motivo]);
            header('Location: /admin/utenti.php?msg=bannato'); exit;
        case 'sbanna':
            $s = $conn->prepare('UPDATE utente SET bannato=0, ban_motivo=NULL WHERE id=?');
            $s->bind_param('i', $target_id); $s->execute(); $s->close();
            admin_log($conn, 'unban_utente', 'utente', $target_id);
            header('Location: /admin/utenti.php?msg=sbannato'); exit;
        case 'elimina':
            $s = $conn->prepare('DELETE FROM utente WHERE id=? AND role != "admin"');
            $s->bind_param('i', $target_id); $s->execute(); $s->close();
            admin_log($conn, 'elimina_utente', 'utente', $target_id);
            header('Location: /admin/utenti.php?msg=eliminato'); exit;
    }
    header('Location: /admin/utenti.php'); exit;
}

// ── Da qui in poi: solo output HTML ───────────────────────────────────
...
$flash_map = [
    ...
    'selfAction'     => ['danger',  'Non puoi agire sul tuo stesso account.'],
...
$listStmt = $conn->prepare(
    "SELECT id,nome,cognome,email,role,bannato,ban_motivo,verified,created_at
     FROM utente $where ORDER BY created_at DESC LIMIT ? OFFSET ?");
...
<main class="m-page"><div class="m-container">
  <div class="m-page-head">
    <div><h1>Gestione utenti</h1><div class="m-page-head__sub"><?= $total ?> account totali</div></div>
  ...
  <form method="get" class="adm-search">
    <input type="search" name="q" value="<?= htmlspecialchars($search) ?>" placeholder="Cerca nome, cognome o email…" class="adm-search__input">
    <button type="submit" class="m-btn">Cerca</button>
    <?php if ($search): ?><a href="/admin/utenti.php" class="m-btn m-btn--ghost">Tutti</a><?php endif; ?>
  </form>
  <div class="adm-table-wrap"><table class="adm-table">
    <thead><tr><th>#</th><th>Nome</th><th>Email</th><th>Ruolo</th><th>Stato</th><th>Registrato</th><th>Azioni</th></tr></thead>
    <tbody>
    ...
    <?php else: while ($u = $utenti->fetch_assoc()): ?>
      <tr class="<?= $u['bannato'] ? 'adm-row--banned' : '' ?>">
        ...
        <td><?= $u['role']==='admin' ? '<span class="adm-badge adm-badge--admin">Admin</span>' : '<span class="adm-badge">User</span>' ?></td>
        <td><?php
          if ($u['bannato']) echo '<span class="adm-badge adm-badge--danger" title="'.htmlspecialchars($u['ban_motivo']??'').'">Bannato</span>';
          elseif (!$u['verified']) echo '<span class="adm-badge adm-badge--warn">Non verificato</span>';
          else echo '<span class="adm-badge adm-badge--ok">Attivo</span>';
        ?></td>
        <td><?= date('d/m/Y', strtotime($u['created_at'])) ?></td>
        <td><?php if ($u['role'] !== 'admin'): ?>
          <div class="adm-row-actions">
            <?php if ($u['bannato']): ?>
              <form method="post" action="/admin/utenti.php"><?= csrf_field() ?>
                <input type="hidden" name="action" value="sbanna">
                <input type="hidden" name="user_id" value="<?= (int)$u['id'] ?>">
                <button class="adm-btn adm-btn--ok">Sbanna</button></form>
            <?php else: ?>
              <form method="post" action="/admin/utenti.php" class="adm-ban-form"><?= csrf_field() ?>
                <input type="hidden" name="action" value="banna">
                <input type="hidden" name="user_id" value="<?= (int)$u['id'] ?>">
                <input type="text" name="motivo" placeholder="Motivo" class="adm-input-sm" required maxlength="200">
                <button class="adm-btn adm-btn--warn">Banna</button></form>
            <?php endif; ?>
            <form method="post" action="/admin/utenti.php" onsubmit="return confirm('Eliminare definitivamente?')"><?= csrf_field() ?>
              <input type="hidden" name="action" value="elimina">
              <input type="hidden" name="user_id" value="<?= (int)$u['id'] ?>">
              <button class="adm-btn adm-btn--danger">Elimina</button></form>
          </div>
        <?php else: ?><span class="adm-ip">—</span><?php endif; ?></td>
      </tr>
    <?php endwhile; endif; ?>
    </tbody>
  </table></div>
